...
class HyperPcViewMoysklad_Service extends ViewLegacy
{
        ...
        if (empty($this->item->id)) {
            ...
            $this->hyper['cms']->redirect(
                $this->hyper['route']->build([
                    'view' => 'positions',
                    'folder_id' => $this->hyper['input']->get('product_folder_id', 1)
                ])
            );
        }
    ...
    {
        ToolbarHelper::apply('moysklad_service.apply');
        ToolbarHelper::save('moysklad_service.save');
        ...
        ToolbarHelper::cancel('moysklad_service.cancel');

        ToolbarHelper::title(Text::sprintf('COM_HYPERPC_VIEW_SERVICES_EDIT_TITLE', $this->item->name, $this->item->sale_price->text()), 'puzzle');
    ...
    protected function _loadAssets()
    {
        ...
        $this->hyper['helper']['assets']->addScript('
             Joomla.submitbutton = function(task) {
                if (task == "moysklad_service.cancel" || document.formvalidator.isValid(document.getElementById("item-form"))) {
