...
// Предварительно при отправке формы редактирования телефона/почты сохраняем в сессию была ли пройдена капча при отправке формы редактирования с этими данными.
// Если да, выводим форму без капчи. Если нет, то с капчей. Иначе можно будет бесконтрольно отправлять смс используя куки одной сессии
...
<form class="jsMergeConfirmForm">
    ...
    <div class="uk-alert uk-alert-warning">
        <?= Text::sprintf('HYPER_ELEMENT_AUTH_MOBILE_EDIT_ACCOUNT_FORM_MERGE_CONFIRM_TEXT', $editValue) ?>
    </div>
    ...
    <div class="uk-text-center">
        <button type="submit" class="uk-width-1-1 uk-button uk-button-primary uk-button-large@s uk-margin">
            <?= Text::_('HYPER_ELEMENT_AUTH_MOBILE_EDIT_ACCOUNT_FORM_MERGE_CONFIRM_SUBMIT') ?>
        </button>
        <button type="button" class="jsMergeConfirmBack uk-button uk-button-text" data-type="<?= $this->getType() ?>">
            <?= Text::_('HYPER_ELEMENT_AUTH_MOBILE_EDIT_ACCOUNT_FORM_MERGE_CONFIRM_BACK') ?>
        </button>
    </div>
</form>
