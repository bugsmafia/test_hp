...
$isInCompare    = array_key_exists($itemCompareKey, (array) $compareItems);
$title          = $isInCompare ? 'COM_HYPERPC_CONFIGURATOR_COMPARE_REMOVE' : 'COM_HYPERPC_CONFIGURATOR_COMPARE_ADD';

$itemKey = $part->getItemKey();
$itemKeySegments = \explode('-', $itemKey);
...
$buttonAttrs = [
    ...
    'title' => Text::_($title),
    'data' => [
        'id'        => $itemKeySegments[1],
        'option-id' => $itemKeySegments[2] ?? null,
        'type'      => $itemKeySegments[0],
        'itemKey'   => $itemKey
    ]
];

$compareGroupId = $part->getFolderId();
$notebookGroups = (array) $this->hyper['params']->get('notebook_groups');
if (\in_array((string) $compareGroupId, $notebookGroups)) {
    $compareGroupId = GROUP_ID_NOTEBOOK_ROOT; /** @todo fix notebook group */
}

$buttonAttrs['data']['group'] = $compareGroupId;
?>
